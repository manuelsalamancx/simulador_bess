...
class CalculosAuditoriaScreen extends StatefulWidget {
  const CalculosAuditoriaScreen({super.key});

  @override
  State<CalculosAuditoriaScreen> createState() => _CalculosAuditoriaScreenState();
}

class _CalculosAuditoriaScreenState extends State<CalculosAuditoriaScreen> with WidgetsBindingObserver {
  ...
  Future<void> _calcularTodo() async {
    ...
    potencia = double.tryParse(prefs.getString('potencia_fv_aud')?.replaceAll(',', '.') ?? '20.0') ?? 20.0;
    rendimiento = double.tryParse(prefs.getString('rendimiento_aud')?.replaceAll(',', '.') ?? '0.8') ?? 0.8;
    limiteIny = double.tryParse(prefs.getString('limite_iny_aud')?.replaceAll(',', '.') ?? '15.0') ?? 15.0;
    mercadoGlobalActivo = prefs.getBool('mercado_activo_aud') ?? true;
    ...
    for (String m in meses) {
      ...
      List<String>? radGuardada = prefs.getStringList('radiacion_aud_$m');
  ...
  Future<void> _simularMesPantalla(String mes, List<int> hList) async {
    final prefs = await SharedPreferences.getInstance();
    List<String>? radGuardada = prefs.getStringList('radiacion_aud_$mes');
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            children: [
              Text('DATOS DE AUDITORÍA (MWh)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.blueGrey)),
        ...
        children: [
          Expanded(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('ANÁLISIS DE AUDITORÍA', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.blueGrey)),
